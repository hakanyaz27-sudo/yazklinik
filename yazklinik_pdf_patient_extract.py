...
def _detect_pdf_report_type(
        folded_text: str,
        voluson_ob_report: bool = False,
        fetal_values: dict[str, Any] | None = None,
        lab_count: int = 0) -> str:
    folded = _fold(str(folded_text or ""))
    fetal_values = dict(fetal_values or {})

    if voluson_ob_report:
        return "obstetric"

    obstetric_metric_keys = (
        "ga_weeks", "ga_days", "bpd", "hc", "ac", "fl", "crl", "nt",
        "efw", "afi", "fhr", "ua_pi", "ua_ri", "ua_sd", "mca_pi",
        "mca_ri", "mca_psv", "uta_pi", "uta_ri", "dv_pi", "cpr",
    )
    has_obstetric_metrics = any(
        fetal_values.get(k) not in (None, "")
        for k in obstetric_metric_keys)
    if has_obstetric_metrics:
        return "obstetric"
    ...
    if any(tok in folded for tok in obstetric_strong):
        return "obstetric"

    obstetric_soft = (
        "ga(lmp)", "edd(lmp)", "fetus", "gravida", "para", "lmp", "date of exam",
    )
    gynecologic_tokens = (
        "gynecology report",
        "gynaecology report",
    ...
    ob_hits = sum(1 for tok in obstetric_soft if tok in folded)
    gy_hits = sum(1 for tok in gynecologic_tokens if tok in folded)
    if ob_hits >= 3 and ob_hits >= (gy_hits + 1):
        return "obstetric"
    if gy_hits >= 1 and ob_hits == 0:
        return "gynecologic"
    if int(lab_count or 0) > 0 and ob_hits == 0 and gy_hits == 0:
        return "lab"
    if gy_hits >= 1:
        return "gynecologic"
    return ""
...
def extract_patient_pdf_payload(pdf_items: Iterable[dict[str, Any]]) -> dict[str, Any]:
    ...
    for item in pdf_items:
        ...
        raw_path = _clean(item.get("path") or "", limit=400)
        visit_key = _clean(item.get("visit_key") or "", limit=80)
        visit_date = _clean(item.get("visit_date") or item.get("report_date") or "", limit=40)
        if source:
            dedupe_key = f"name:{source.lower()}"
        elif raw_path:
            dedupe_key = f"path:{raw_path.lower()}"
        else:
            dedupe_key = f"fallback:{visit_key.lower()}|{visit_date}"
        if dedupe_key in seen_pdf_inputs:
            continue
        seen_pdf_inputs.add(dedupe_key)
        ...
        report_type = _detect_pdf_report_type(
            folded_text=folded,
            voluson_ob_report=voluson_ob_report,
            fetal_values=pdf_fetal,
            lab_count=len(pdf_labs),
        )
        if report_type:
            pdf_record["report_type"] = report_type
