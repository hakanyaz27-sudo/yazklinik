...
DB_PATH = r"D:\YazKlinik_Final_D500\local_db\yazklinik_v68.sqlite3"
...
def download_fresh(out_dir: str) -> dict:
    ...
    # D700: tum klinik exportlar alinir. Hasta/protokol zorunlu;
...
def import_obstetri_xlsx(con: sqlite3.Connection, path: str) -> tuple[int, int]:
    ...
    cols = {
        ...
        "ta": _idx("Ta"),
        "sikayet": _idx("Åikayet"),
        "olusturulma": _idx("OluÅŸturulma Tarihi") or _idx("OLuÅŸturulma Tarihi"),
        "guncelleme": _idx("GÃ¼ncelleme Tarihi"),
    }
...
def import_tahsilatlar_csv(con: sqlite3.Connection, path: str) -> int:
    ...
    needed = ("Protokol_No", "Hasta_No", "Hasta_AdÄ±", "Hasta_SoyadÄ±",
              "Ã–denen_Miktar", "Cinsi", "Tarihi")
    ...
    for row in rows:
        ...
        hno = row[H["Hasta_No"]]
        had = row[H["Hasta_AdÄ±"]]
        hsd = row[H["Hasta_SoyadÄ±"]]
        odn = row[H["Ã–denen_Miktar"]]
        cns = row[H["Cinsi"]]
        trh = row[H["Tarihi"]]
...
def run_import(since_days: int, dry_run: bool,
    ...
    if exports_dir is None:
        ...
        exports_dir = rf"D:\YazKlinik_Final_D500\bk_exports\{stamp}_import"
    ...
    if not dl.get("ok"):
        LAST_ERROR = str(dl.get("error") or "CSV/XLSX indirilemedi")
        _safe_print(f"  HATA: {LAST_ERROR}")
        return 1
    ...
    needed = ("Protokol No", "Ä°sim", "Soyisim", "Baba AdÄ±", "Anne AdÄ±",
              "Anne TC No", "AnlaÅŸmalÄ± Kurum", "BranÅŸ", "Doktor",
              "Hasta No", "Protokol Tarihi", "Protokol Tipi", "GeliÅŸ Nedeni")
    missing = [c for c in needed if c not in H]
    if missing:
        LAST_ERROR = f"Eksik protokol kolonlari: {', '.join(missing)}"
    ...
    if "Hasta No" not in HH:
        LAST_ERROR = "Hastalar.csv icinde 'Hasta No' kolonu yok"
    ...
    if mode == "missing":
        try:
            con0 = __import__("yazklinik_db_adapter").agent_connection(sqlite_path=DB_PATH)  # PG-primary aware
            db_existing_nos = set(
    ...
    if dry_run:
        ...
        for p in selected_patients[:3]:
            _safe_print(f"    {p.get('AdÄ±','')} {p.get('SoyadÄ±','')} "
                  f"(No: {p.get('Hasta No')}, TC: {p.get('Kimlik NumarasÄ±','')})")
        _safe_print(f"\n  ORNEK 3 protokol:")
        for p in recent_protocols[:3]:
            _safe_print(f"    #{p['Protokol No']} {p['Ä°sim']} {p['Soyisim']} - "
    ...
    for p in selected_patients:
        ...
        params = (
            hno,
            p.get("Kimlik NumarasÄ±", ""),
            p.get("AdÄ±", ""),
            p.get("SoyadÄ±", ""),
            p.get("Cinsiyeti", ""),
            p.get("UyruÄŸu", ""),
            p.get("Pasaport NumarasÄ±", ""),
            p.get("GeliÅŸ Tarihi", ""),
            p.get("Ã–zgeÃ§miÅŸ", ""),
            p.get("SoygeÃ§miÅŸ", ""),
            p.get("Alerjiler", ""),
    ...
    for p in recent_protocols:
        ...
        existing = con.execute(
            ...
            (pno,)).fetchone()
        params = (pno, p["Hasta No"], p["Ä°sim"], p["Soyisim"],
                  p["Baba AdÄ±"], p["Anne AdÄ±"], p["Anne TC No"],
                  p["AnlaÅŸmalÄ± Kurum"], p["BranÅŸ"], p["Doktor"],
                  p["Protokol Tarihi"], p["Protokol Tipi"],
                  p["GeliÅŸ Nedeni"], now_iso)
        con.execute("""
    ...
    # D700 clinical export katmani: BK'daki gelis iceriÄŸi, hizmet,
    # randevu ve jinekoloji recete/takip bilgileri Voluson tarafinda
    # okunabilir hale gelsin diye yerel tablolara alinÄ±r.
    ...
    try:
        ...
        LAST_ERROR = f"BK->Voluson mirror hatasi: {ex}"
