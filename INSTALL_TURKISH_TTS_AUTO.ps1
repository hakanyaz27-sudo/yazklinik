# YazKlinik D700 - Windows TR TTS Auto-installer (no prompt)
# Caglrilabilirligi: Start-Process powershell -Verb RunAs -ArgumentList "-NoProfile -ExecutionPolicy Bypass -File <bu_dosya>"
# Log: D:\YazKlinik_Final_D500\INSTALL_TURKISH_TTS_AUTO.log

$ErrorActionPreference = "Continue"
$logPath = "D:\YazKlinik_Final_D500\INSTALL_TURKISH_TTS_AUTO.log"
...
if (-not $isAdmin) {
    ...
    "INSTALL_FAIL_NOT_ADMIN" | Out-File "D:\YazKlinik_Final_D500\INSTALL_TURKISH_TTS_AUTO.status" -Encoding ASCII -Force
...
try {
    ...
    if ($trVoices.Count -gt 0) {
        ...
        "INSTALL_OK_TR_VOICES=$($trVoices.Count)" | Out-File "D:\YazKlinik_Final_D500\INSTALL_TURKISH_TTS_AUTO.status" -Encoding ASCII -Force
    } else {
        Warn "Capabilities yuklendi ama SAPI'de TR ses gorunmedi. Yeniden baslatma gerekebilir."
        "INSTALL_PARTIAL_REBOOT_NEEDED" | Out-File "D:\YazKlinik_Final_D500\INSTALL_TURKISH_TTS_AUTO.status" -Encoding ASCII -Force
    }
} catch {
    Fail "SAPI son kontrol hata: $_"
    "INSTALL_FAIL_SAPI" | Out-File "D:\YazKlinik_Final_D500\INSTALL_TURKISH_TTS_AUTO.status" -Encoding ASCII -Force
}
