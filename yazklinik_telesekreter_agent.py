...
PHONE_CUE_KEYWORDS = (
    "telefon", "numara", "numarasi", "numaram", "beni ara", "arayin",
    "ulas", "whatsapp",
)
...
def _normalize_callback_phone(value: Any) -> Optional[str]:
    """Normalize TR callback phone to 10-digit national form when possible."""
...
def extract_callback_phone(transcript: str, caller_phone: str) -> Optional[str]:
    """Konusmadan mantikli callback telefonu ayikla; tarih/saat ile karistirma."""
    text = str(transcript or "")
    # 1) TR telefon formatlari (+90/0, bosluk-tire-nokta) -> guvenli aday.
    patterns = (
        r"(?<!\d)(?:\+?90[\s.\-]*)?(?:0[\s.\-]*)?5\d(?:[\s.\-]*\d){9}(?!\d)",
        r"(?<!\d)(?:\+?90[\s.\-]*)?(?:0[\s.\-]*)?[2-9]\d(?:[\s.\-]*\d){8,9}(?!\d)",
    )
    for pat in patterns:
        for match in re.finditer(pat, text):
            normalized = _normalize_callback_phone(match.group(0))
            if normalized:
                return normalized

    # 2) Ayiricisiz uzun rakam blogu (10-13) gorulurse normalize et.
    for match in re.finditer(r"(?<!\d)\d{10,13}(?!\d)", text):
        normalized = _normalize_callback_phone(match.group(0))
        if normalized:
            return normalized

    # 3) Ancak telefon kelimesi geciyorsa, yakin cevre rakamlarindan dene.
    folded = _fold(text)
    if any(cue in folded for cue in PHONE_CUE_KEYWORDS):
        all_digits = _digits_only(text)
        if len(all_digits) >= 10:
            tail = all_digits[-13:]
            normalized = _normalize_callback_phone(tail)
            if normalized:
                return normalized

    # 4) Son fallback: caller-id
    fallback = _normalize_callback_phone(caller_phone)
    if fallback:
        return fallback
    caller_digits = _digits_only(caller_phone)
    if len(caller_digits) >= 10:
        return caller_digits[-10:]
    return None
