# YazKlinik D700 - Restic Gunluk Otomatik Yedek
# Task Scheduler ile her gun 03:00'te calistir:
#   schtasks /create /tn "YazKlinik Restic Daily" /tr "powershell.exe -ExecutionPolicy Bypass -File D:\YazKlinik_Final_D500\akillilik\scripts\restic_daily_backup.ps1" /sc daily /st 03:00 /rl HIGHEST
#
...
& $resticBin backup `
    ...
    "$root\static" `
    "$root\config.env" `
    --tag $tag --tag d700-daily 2>&1 | Out-File $log -Append
$bk = $LASTEXITCODE
