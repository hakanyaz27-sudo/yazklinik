...
REM YazKlinik Final D250 - D128 tipi Windows WebShell
...
set "DEFAULT_SERVER=https://127.0.0.1:5443"
set "SERVER_URL=%~1"
...
if /I not "%YAZKLINIK_FORCE_QT_WEBSHELL%"=="1" (
  ...
  if defined BROWSER_EXE (
    ...
    start "YazKlinik WebShell" "%BROWSER_EXE%" --app="%SERVER_URL%/?yk_webshell=1" --user-data-dir="%LOCALAPPDATA%\YazKlinik\BrowserShell" --disk-cache-dir="%LOCALAPPDATA%\YazKlinik\BrowserShellCache" --disk-cache-size=536870912 --media-cache-size=268435456 "--user-agent=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0 Safari/537.36 YazKlinikWebShell/1.0" --no-first-run --no-default-browser-check --disable-extensions --disable-sync --disable-background-networking --disable-component-update --disable-domain-reliability --disable-notifications --disable-search-engine-choice-screen --disable-features=Translate,MediaRouter,CalculateNativeWinOcclusion --enable-gpu-rasterization --enable-zero-copy --enable-features=CanvasOopRasterization --enable-smooth-scrolling --disable-background-timer-throttling --disable-backgrounding-occluded-windows --disable-renderer-backgrounding --no-proxy-server
    endlocal
    exit /b 0
  )
)

set "PYTHON_EXE=C:\Users\yazha\OneDrive\Desktop\YazKlinik_Final_D104\.venv\Scripts\python.exe"
set "PYTHONW_EXE=C:\Users\yazha\OneDrive\Desktop\YazKlinik_Final_D104\.venv\Scripts\pythonw.exe"
...
echo [3/3] YazKlinik D250 WebShell aciliyor...
