...
def _bk_text_scores(text: str) -> dict:
    t = _ascii_text(_text(text))
    if not t:
        return {"rx": 0, "history": 0, "dx": 0, "plan": 0}
    rx_kw = (
        " tb", " tablet", " kapsul", " ampul", " mg", " ml", " doz", "gun",
        "x1", "1x1", "2x1", "3x1", "1x2", "bid", "tid", "po", "iv", "im",
        "sc", "vajinal", "rektal", "sabah", "aksam", "tok", "ac karna",
        "kullan", "recete", "ilac"
    )
    hx_kw = (
        "sikayet", "oyku", "hikaye", "anamnez", "yaknma", "yakn", "kanama",
        "adet", "agr", "agri", "gecikme", "lekelenme", "takip", "muayene"
    )
    dx_kw = ("tani", "icd", "n92", "n93", "o", "kist", "myom", "pid", "endometr")
    plan_kw = ("plan", "oner", "kontrol", "tetkik", "beta hcg", "usg", "izlem", "takibi")
    ...
    return {
        "rx": _count(t, rx_kw),
        "history": _count(t, hx_kw),
        "dx": _count(t, dx_kw),
        "plan": _count(t, plan_kw),
    }


def _bk_is_prescription_like(text: str) -> bool:
    raw = _text(text)
    if not raw:
        return False
    s = _bk_text_scores(raw)
    short_rx_pattern = bool(re.search(r"\b([1-4]x[1-4]|bid|tid)\b", _ascii_text(raw)))
    if short_rx_pattern and s["rx"] >= 1:
        return True
    return s["rx"] >= 2 and s["rx"] >= (s["history"] + 1)


def _bk_semantic_pack_gyn(gyn: dict, fallback_dx: str = "") -> dict:
    complaint = _text(gyn.get("sikayet_oyku"))
    diagnosis = _text(gyn.get("tani")) or _text(fallback_dx)
    plan = _text(gyn.get("tedavi_plani"))
    rx_raw = _text(gyn.get("recete"))
    notes = _text(gyn.get("notlar"))

    # Hatali map durumlari: hikaye/plan recete alanina dusmus olabilir.
    if rx_raw and not _bk_is_prescription_like(rx_raw):
        sc = _bk_text_scores(rx_raw)
        if sc["history"] >= max(sc["rx"], sc["dx"], sc["plan"]):
            complaint = complaint or rx_raw
        elif sc["plan"] >= max(sc["rx"], sc["history"], sc["dx"]):
            plan = plan or rx_raw
        elif sc["dx"] >= max(sc["rx"], sc["history"], sc["plan"]):
            diagnosis = diagnosis or rx_raw
        else:
            notes = notes or rx_raw
        rx_raw = ""

    # Plan alaninda aslinda recete semasi varsa recete olarak kullan.
    if not rx_raw and plan and _bk_is_prescription_like(plan):
        rx_raw = plan
        plan = ""
...
def _build_protocol_note(con, protocol: dict, display_name: str) -> tuple[str, str, str]:
    ...
    for g in gyn_rows[:20]:
        packed = _bk_semantic_pack_gyn(g)
        line = " | ".join(x for x in (
            _text(g.get("tarih")),
            f"Sikayet/oyku: {packed.get('complaint')}" if packed.get("complaint") else "",
            f"Tani: {packed.get('diagnosis')}" if packed.get("diagnosis") else "",
            f"Tedavi: {packed.get('plan')}" if packed.get("plan") else "",
            f"Recete: {packed.get('prescription')}" if packed.get("prescription") else "",
            f"Not: {packed.get('notes')}" if packed.get("notes") else "",
        ) if x)
        if line:
            gyn_section_items.append(line)
    _append_section(lines, "Jinekoloji", gyn_section_items)
    ...
    for g in gyn_rows:
        packed = _bk_semantic_pack_gyn(g)
        if packed.get("plan"):
            control_items.append(packed.get("plan"))
...
def _upsert_bk_prescription(con: sqlite3.Connection, patient_key: str,
    ...
    if not medications:
        # Akilli siniflandirma sonrasinda recete yoksa eski hatali aynalamayi pasife al.
        if existing:
            con.execute("""
                UPDATE prescriptions
                SET deleted_at=?, deleted_by=?, deleted_reason=?, updated_at=?
                WHERE id=?
            """, (now, "bulutklinik", "auto:non_prescription_text", now, existing["id"]))
        return False
    if existing:
        con.execute("""
            UPDATE prescriptions
...
def _sync_bk_clinical_timeline(con: sqlite3.Connection, bk_no: str,
    ...
    for protocol in protocols:
        ...
        for gyn in _gyn_for_protocol(con, pno):
            packed = _bk_semantic_pack_gyn(gyn, diagnosis)
            if _upsert_bk_prescription(
                    con, patient_key, visit_key, pno, display_name,
                    packed.get("diagnosis") or diagnosis,
                    packed.get("prescription"), now):
    ...
    for gyn in gyn_rows:
        ...
        packed = _bk_semantic_pack_gyn(gyn)
        ...
        note = "\n".join(lines)
        control_note = "\n\n".join(x for x in (
            packed.get("plan"), _text(gyn.get("sonuc"))
        ) if x)
        visit_path = os.path.join(full_path or _virtual_path(patient_key), visit_key)
        if _upsert_bk_visit(con, patient_key, visit_key, visit_path,
        ...
        if _upsert_bk_prescription(
                ...
                display_name, packed.get("diagnosis"),
                packed.get("prescription"), now):
            stats["prescriptions"] += 1
