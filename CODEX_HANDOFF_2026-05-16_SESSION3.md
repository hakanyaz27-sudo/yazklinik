# CODEX HANDOFF â€” Session 3 (2026-05-16 aksam)
...
| 2 | `3cbed73` | Codex | Hide D700 background services on startup |
...
- **ProxyFix middleware** â€” `X-Forwarded-For/Proto/Host/Port` basliklarina guven
  (reverse proxy / Tailscale Funnel arkasinda dogru URL/IP gorulsun)
- **`is_remote_request(request)` helper** â€” LAN, Tailscale (100.x), IPv6 link-local
  lokal sayilir; public IP'ler `True` doner
- **`@app.before_request`** â€” uzak istekleri `logger.info()` ile yazar
- **`@app.after_request`** â€” `X-YK-Server: D700` + `X-YK-Remote-Detected: 1` header'lari

Cift register koruma: `app.wsgi_app._yk_proxy_fixed` marker.

### `yazklinik_web.py` â€” Flask app olusturmasi sonrasi 9 satir ekleme

```python
app = Flask(__name__)

# === D700 2026-05-16: Reverse proxy / Tailscale Funnel destegi ===
try:
...
### `yazklinik_sip_alex_client.py` â€” Alex SIP bridge (1146 satir)
...
### `D500_SERVICE_RUNNER.py` + `D500_SIP_ALEX_BASLAT.bat` â€” bg services hide
...
| Response header'lari | `X-YK-Server: D700` + `X-YK-Remote-Detected` |
...
# Beklenen: X-YK-Server: D700  (X-YK-Remote-Detected YOK)

# Manifest
python -c "from yazklinik_feature_sync import MANIFEST_VERSION; print(MANIFEST_VERSION)"
# Beklenen: 2026.05.16-D700-AGENTS-IG-CEVIRI-KONSULT
```
...
3cbed73  Hide D700 background services on startup  (Codex)
...
- `TAILSCALE_UZAKTAN_ERISIM.md` (yeni, 310 satir)
- `D500_HANDOFF.md` (adres bloguna 3 satir)
...
- `D500_SERVICE_RUNNER.py` (yeni)
- `D500_SIP_ALEX_BASLAT.bat` (yeni, 65 satir)
