...
def scan_archive(root: str, max_candidates: int = 60, include_pdf: bool = True,
    ...
    exclude_paths: bu pathler skip edilir (kullanicinin onceden gordugu/sectigi
                   dosyalar tekrar gozukmesin).
    """
    # exclude set - hizli lookup
    exclude_set = set()
    if exclude_paths:
        for p in exclude_paths:
            if p:
                exclude_set.add(str(p).strip().lower().replace("/", "\\"))
    root_path = Path(root)
    ...
    for path in _walk_files(root_path, include_pdf=include_pdf, max_files=DEFAULT_WALK_MAX_FILES):
        ...
        # exclude_paths kontrolu - kullanici onceden gordugu/sectigi
        path_norm = str(path).strip().lower().replace("/", "\\")
        if path_norm in exclude_set:
            skipped["excluded_by_user"] = skipped.get("excluded_by_user", 0) + 1
            continue
        try:
            st = path.stat()
