...
chcp 65001 >nul
title YazKlinik D300 - TURBO Temizleyici + Restart
cd /d "%~dp0"
cls

echo.
echo  ============================================================
echo            YazKlinik D300 - TURBO Mode (RAM + Cache + DB)
echo  ============================================================
...
REM ---------------------------------------------------------------
REM 3) Eski server'i durdur (port 5443)
REM ---------------------------------------------------------------
echo  [3/7] Eski server (port 5443) durduruluyor...
set "OLD_PID="
for /f "tokens=5" %%a in ('netstat -ano ^| findstr ":5443 " ^| findstr "LISTENING"') do set "OLD_PID=%%a"
if defined OLD_PID (
  echo      PID %OLD_PID% durduruluyor...
  taskkill /PID %OLD_PID% /F >nul 2>&1
  timeout /t 2 /nobreak >nul
) else (
  echo      Port 5443 zaten bos.
)
...
set "DB_PATH=D:\YazKlinik_Final_D300\local_db\yazklinik_v68.sqlite3"
...
echo.
echo  WebShell acmak istersen: D300_BASLAT.bat veya
