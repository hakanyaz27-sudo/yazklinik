...
_MOJIBAKE_MARKER_RE = re.compile("[" + re.escape("".join(_MOJIBAKE_MARKERS)) + "]")


def _mojibake_score(text: str) -> int:
    # Perf (D700): tek regex taramasi tum tek-karakter marker'lari sayar.
    # markers ayri tek karakterler oldugu icin len(findall([markers])) degeri
    # eski "sum(text.count(m) for m in _MOJIBAKE_MARKERS)" ile bire bir ayni;
    # boylece 14 ayri str.count taramasi tek tarama olur. Sekans marker'lari
    # (cok karakterli) ayri sayilip x4 agirlikli kalir. Donus degeri ozdes.
    # Tum marker/sekans karakterleri > 0x7F oldugu icin saf ASCII metnin skoru
    # kesin 0'dir; isascii() tek C-seviyesi tarama ile cogu segmenti (bosluk/
    # ASCII kelime/ASCII tag) regex+count'a hic sokmadan eler.
    if text.isascii():
        return 0
    return (
        len(_MOJIBAKE_MARKER_RE.findall(text))
...
def _final_cleanup(value: str) -> str:
    ...
    for old, new in extra_replacements.items():
        value = value.replace(old, new)
    # Additional high-frequency UI fragments (WebShell / mixed encoding traces)
    # kept as unicode escapes so source stays ASCII-safe.
    value = value.replace("Geli\u00c3\u0178", "Gelis")
    value = value.replace("Geli\u00c5\u0178", "Gelis")
    value = value.replace("geli\u00c3\u0178", "gelis")
    value = value.replace("geli\u00c5\u0178", "gelis")
    value = value.replace("Geli\u00c3\u0178ler", "Gelisler")
    value = value.replace("Geli\u00c5\u0178ler", "Gelisler")
    value = value.replace("geli\u00c3\u0178ler", "gelisler")
    value = value.replace("geli\u00c5\u0178ler", "gelisler")
    value = value.replace("Do\u00c3\u0178um", "Dogum")
    value = value.replace("Do\u00c4\u0178um", "Dogum")
    value = value.replace("do\u00c3\u0178um", "dogum")
    value = value.replace("do\u00c4\u0178um", "dogum")
    value = value.replace("ba\u00c5\u0178ar\u00c4\u00b1l\u00c4\u00b1", "basarili")
    value = value.replace("kayd\u00c4\u00b1", "kaydi")
    value = value.replace("Konu\u00c5\u0178", "Konus")
    value = value.replace("konu\u00c5\u0178", "konus")
    value = value.replace("\u00c4\u00b0\u00c5\u0178", "Is")
    value = value.replace("Do\u00c3\u0178uranlar", "Doguranlar")
    value = value.replace("kayd\u00c3\u00b1", "kaydi")
    value = value.replace("ba\u00c5\u0178ar\u00c4\u00b1l\u00c4\u00b1", "basarili")
    value = value.replace("Ayr\u00c3\u00b1nt\u00c3\u00b1", "Ayrinti")
