...
def render_html(rpt: StatusReport) -> str:
    ...
    badge = {"operational": ("#166534", "[OK] ÇALIŞIYOR"),
             "degraded": ("#92400e", "[!!] KISMI"),
             "down": ("#991b1b", "[XX] KESİNTİ")}
    color, txt = badge.get(rpt.overall, ("#666", "?"))
    rows = []
    for s in rpt.services:
        sym = "[OK]" if s.up else "[XX]"
        c = "#166534" if s.up else "#991b1b"
...
.banner small{{font-size:13px;font-weight:400;opacity:1;display:block;margin-top:6px}}
...
td:nth-child(3){{text-align:right;color:#334155;font-variant-numeric:tabular-nums}}
tr:last-child td{{border-bottom:none}}
.foot{{margin-top:20px;color:#334155;font-size:13px;text-align:center;line-height:1.6}}
