...
FEATURE_GROUPS = (
    ...
    ("AJANLAR", (
        _r("/ajanlar", "Klinik Ajanlari", "31 ajan dashboard + manifest", "ai", "[AJ]",
    ...
    ("SESSION 7 (YENI)", (
        ...
        _r("/stok", "Stok Takip", "Ilac + sarf miat + azalan + sipariş onerisi", "system", "[SK]",
        ...
        _r("/api/agents/gebelik/plan", "Gebelik Takvimi", "LMP -> NT/morfo/GDM/GBS/dogum tarihleri", "clinical", "[GT]"),
        _r("/api/agents/risk/preeklampsi", "Preeklampsi Risk Skoru", "SBP/DBP/protein -> evre + tedavi onerisi", "clinical", "[PR]"),
        _r("/api/agents/risk/hellp", "HELLP Risk (Mississippi)", "Hemoliz+AST+trombosit -> Class I/II/III", "clinical", "[HL]"),
        _r("/api/agents/risk/bishop", "Bishop Indukstion Skoru", "Servikal -> indukstion uygunluk", "clinical", "[BP]"),
        _r("/api/agents/risk/vte", "VTE Padua Skoru", "Yatan hasta tromboz riski + LMWH", "clinical", "[VT]"),
        _r("/api/agents/ddi/check", "DDI Ilac Etkilesim", "Receteyi cift cift kontrol + gebelik kategori", "clinical", "[DD]",
           simple=True, aliases=("ilac etkilesim", "drug interaction", "ddi")),
        _r("/api/agents/smear-hpv/followup", "Smear/HPV Takip (ASCCP)", "ASCUS+HPV+ -> kolposkopi tarihi otomatik", "clinical", "[SH]"),
        _r("/api/agents/phq9/score", "PHQ-9 Depresyon Tarama", "9 soru postpartum depresyon + intihar uyarisi", "clinical", "[PQ]"),
        _r("/api/agents/konsey/build", "Konsey Vaka Sunum", "Tumor board markdown slayt + RAG citation", "clinical", "[KS]"),
        _r("/api/agents/hatira-usg/prepare", "Hatira USG WhatsApp", "Guzel kareyi anneye anonim + watermark", "media", "[HU]"),
        _r("/api/agents/voice-command/parse", "Voice Command", "'BPD 85', 'yeni hasta Ayse' regex parse", "ai", "[VC]"),
        _r("/api/agents/burnout/report", "Anti-Burnout Dashboard", "Yorgunluk skoru + mola onerisi", "system", "[AB]"),
        _r("/api/agents/orchestrator/full-visit", "Multi-Agent Orchestrator", "Tam ziyaret zinciri (5 ajan tek tikla)", "ai", "[OR]"),
        _r("/api/agents/portal/issue-link", "Magic-Link Uret", "Hasta self-service icin 24s gecerli link", "patient", "[ML]"),
        _r("/api/agents/payment/initiate", "Iyzico/Stripe Odeme", "Kart tahsilat + 3D Secure + iade", "system", "[PY]"),
        _r("/api/agents/memnuniyet/survey-yesterday", "Memnuniyet Anketi (Dun)", "Dun gelenlere 5-yildiz WhatsApp", "ai", "[MS]"),
        _r("/api/agents/memnuniyet/birthday-today", "Dogum Gunu Tebrik (Bugun)", "Bugun dogan hastalara mesaj", "ai", "[BT]"),
        _r("/api/agents/pubmed-cron/scan", "PubMed Tarama (Gunluk)", "8 sorgu yeni makale + RAG indeks", "ai", "[PC]"),
        _r("/api/agents/medula/provizyon", "Medula Provizyon (Stub)", "SGK sigorta sorgulama iskelet", "system", "[MD]"),
        _r("/api/agents/lab-duzen/fetch", "Duzen Lab Sonuc (Stub)", "Hastanin lab sonucu cek", "clinical", "[LB]"),
        _r("/api/agents/iot/scan", "IoT BT Cihaz Tara (Stub)", "Omron BP / Xiaomi tarti / Accu-Chek", "clinical", "[IO]"),
        _r("/api/agents/plugins/list", "3rd Party Plugin Loader", "plugins/ klasoru dinamik yukleyici", "system", "[PL]"),
    )),
)
