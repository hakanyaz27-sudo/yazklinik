...
chcp 65001 >nul
title YazKlinik D250 - Hizli Baslat
cd /d "%~dp0"
cls

echo.
echo  ============================================================
echo                YazKlinik D250 - Hizli Baslat (Asustor)
echo  ============================================================
echo  Klasor: %CD%
echo.
...
if exist "config.env" (
  ...
  set "YAZKLINIK_DB_PATH=D:\YazKlinik_Final_D250\local_db\yazklinik_v68.sqlite3"
...
REM 2) Hazir .venv bul - D104 paylasim, D250 yerel veya yenide kur
set "VENV_PATH="
...
"%VENV_PATH%\Scripts\python.exe" "%~dp0D250_TERMINAL_SYNC.py" --server-url "%YAZKLINIK_SERVER_URL%" >nul 2>&1
...
start "" /B "%VENV_PATH%\Scripts\python.exe" -u yazklinik_web.py 1>"%STDOUT_LOG%" 2>"%STDERR_LOG%"

REM 7) Hazir olmasini bekle (max 30 sn)
set "READY=0"
for /l %%i in (1,1,30) do (
  if !READY!==0 (
    timeout /t 1 /nobreak >nul
    powershell -NoProfile -Command "[System.Net.ServicePointManager]::ServerCertificateValidationCallback = {$true}; try { (Invoke-WebRequest -Uri '%YAZKLINIK_SERVER_URL%/giris' -UseBasicParsing -TimeoutSec 1 -ErrorAction Stop).StatusCode } catch { exit 1 }" >nul 2>&1
