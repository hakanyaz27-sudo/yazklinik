"""Alex RAG (Retrieval-Augmented Generation) altyapı.

Hasta dosyaları, USG raporları, araştırma özetleri, doktor notları semantic
search ile indekslenir. Alex her sorguda en alakalı 3-5 belgeyi otomatik
prompt'a inject eder.

Embedding model: paraphrase-multilingual-mpnet-base-v2 (Türkçe destekli, ~420MB)
Vector DB: ChromaDB embedded (SQLite backend, server gerek yok)

İlk çalıştırmada model otomatik indirilir (~1-2 dakika), sonra cache'den çalışır.
"""
...
def _yk_inject_ffmpeg_path():
    candidates = [
        r"D:\YazKlinik_Final_D300\models\ffmpeg\extracted",
        r"D:\YazKlinik_Final_D300\tools\ffmpeg",
    ]
...
def _get_model():
    """sentence-transformers modelini lazy yükle."""
    global _MODEL
    ...
    with _LOCK:
        ...
        try:
            ...
            pass
        # CUDA varsa kullan (RTX 5090'da ~5x hızlı)
        device = "cuda"
        try:
            import torch
...
def _get_collection():
    ...
    with _LOCK:
        ...
        _COLLECTION = _CLIENT.get_or_create_collection(
            ...
            metadata={"description": "Alex bilgi tabanı: hasta, USG, araştırma"})
    return _COLLECTION
...
def embed_batch(texts):
    """Birden çok text -> embedding listesi (batch, hızlı)."""
    if not texts:
        return []
    m = _get_model()
...
def format_results_for_prompt(results, max_chars_each=400):
    """RAG sonuçlarını prompt'a uygun string formatına çevir."""
    if not results:
        return ""
