...
AGENT_VERSION = "2026.05.20-integration-agents-medgemma"
...
INTEGRATION_AGENTS = [
    ...
    {
        "id": "medgemma",
        "name": "MedGemma Klinik YZ Ajani",
        "short": "Google MedGemma modelini lokal/ozel endpoint olarak YazKlinik'e baglar; USG/PDF/lab/not icin doktor onayli medikal taslak uretir.",
        "status": "ready_when_model_installed",
        "risk": "high",
        "directions": ["YazKlinik klinik veri -> lokal MedGemma karar destek taslagi"],
        "module": "yazklinik_medgemma_agent",
        "entry_function": "analyze",
        "panel_route": "/medgemma-klinik-yz",
        "api_endpoints": [
            "GET  /api/agents/medgemma/health",
            "POST /api/agents/medgemma/analyze",
        ],
        "safe_methods": [
            "Varsayilan kapali; model/endpoint hazir degilse sadece durum ve prompt onizleme verir",
            "Uzak endpoint varsayilan bloklu; YAZKLINIK_MEDGEMMA_ALLOW_REMOTE=1 olmadan hasta verisi cikmaz",
            "Cikti tani/tedavi emri degil, doktor onayli karar destek taslagi olarak isaretlenir",
            "USG/PDF/lab metninde eksik bilgi ve kirmizi alarm ayrilir",
            "Hugging Face model terms onayi ve token gerekliligi durum ekraninda gosterilir",
        ],
        "blocked_methods": [
            "Doktor onayi olmadan hasta dosyasina otomatik kayit",
            "Uzak bulut endpoint'e sessiz hasta verisi gonderme",
            "Model kurulu degilken web thread'ini agir download ile kilitleme",
        ],
        "doctor_actions": [
            "/medgemma-klinik-yz sayfasinda durum kontrol edilir",
            "HF terms kabul edilip model kurulursa YAZKLINIK_MEDGEMMA_ENABLED=1 yapilir",
            "Klinik not, USG/PDF veya lab metni yapistirilir",
            "Cikti hekim tarafindan kontrol edilip gerekirse hasta dosyasina manuel eklenir",
        ],
    },
]
