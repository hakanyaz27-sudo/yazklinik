...
def _configure_qt_runtime() -> None:
    ...
    if media_origins:
        required_flags.extend([
            ...
            "--unsafely-treat-insecure-origin-as-secure=" + ",".join(media_origins),
        ])
    existing = os.environ.get("QTWEBENGINE_CHROMIUM_FLAGS", "").strip()
    disabled_for_voice = {"--disable-speech-api"}
    parts = [
...
APP_VERSION = "YazKlinik Final D250 Terminal Kabuk"
...
FALLBACK_GROUPS: list[tuple[str, list[tuple[str, str, str]]]] = [
    ("Merkez", [
        ...
        ("/akilli-bildirimler", "Akıllı Bildirimler", "Bugünün takip ve hatırlatmaları"),
        ("/akilli-rehber", "Akıllı Rehber", "Kullanımdan öğrenir"),
    ]),
    ("Klinik", [
        ...
        ("/recete-sablonlari", "Hazır Reçeteler", "Database reçete kütüphanesi"),
    ...
    ("YZ Asistanlar", [
        ...
        ("/akilli-rehber", "Akıllı Rehber", "Yol gösterici asistan"),
        ("/yz-komut-merkezi", "YZ Komut", "Güvenli komut merkezi"),
    ]),
    ("Ses ve Akıllı Diyalog", [
        ("/ses-ve-alex", "Ses ve Alex Merkezi", "Akıllı Diyalog ve ses ayarları tek yerde"),
        ("/akilli-dialog", "Akıllı Diyalog", "Konuşan ve iş yapan YZ ajan"),
        ("/sessiz-alex", "Sessiz Alex", "Klavye ile sessiz Akıllı Diyalog"),
        ("/sesli-recete", "Sesli Reçete", "Konuşarak reçete taslağı"),
        ("/yz-ses-cevir", "Ses Çevir", "Sesli notu metne çevir"),
        ("/ses-profilleri", "Alex Ses Seçimi", "Alex konuşma sesi"),
        ("/mikrofon-tani", "Mikrofon Tanı", "Ses giriş testi"),
        ("/yz-telefon-diyalog", "YZ Diyalog", "Telefonsuz/telefonlu Türkçe sohbet"),
        ("/entegrasyon/yz-telesekreter", "YZ Telesekreter", "IP telefon/PBX"),
    ]),
    ("YZ Analiz ve Otomasyon", [
        ("/islem-zincirleri", "İşlem Zincirleri", "Reçete, randevu ve WhatsApp taslakları"),
        ...
        ("/hasta/{patient}/gorsel-tani-destegi", "Resimle Ön Tanı", "Hasta resmiyle ön tanı ve ilaç ön önerisi"),
    ]),
    ("YZ Ayarlar", [
...
PATIENT_ACTIONS = [
    ...
    ("Akıllı Asistan", "/hasta/{patient}/akilli-asistan"),
    ("Hasta Hafızası", "/hasta/{patient}/hafiza"),
    ("Konuşarak Not", "/hasta/{patient}/konusarak-not"),
    ("Sesli Reçete", "/hasta/{patient}/sesli-recete"),
    ("Akıllı Rehber", "/akilli-rehber"),
    ...
    ("Resimle Ön Tanı", "/hasta/{patient}/gorsel-tani-destegi"),
    ("HD Studio", "/hasta/{patient}/yz-resim-iyilestir"),
    ("HD Studio Medya Sec", "/hasta/{patient}/yz-resim-iyilestir"),
    ("Reçete", "/hasta/{patient}/recete-hazirla"),
    ("Hazır Reçeteler", "/recete-sablonlari?patient={patient}"),
...
class YazKlinikWebMirror(QMainWindow):
    ...
    def _load_finished(self, ok: bool) -> None:
        if ok:
            ...
            return
        if self.connection_mode == "server" and self._switch_to_local_runtime("Sayfa yüklenemedi"):
            self.load_route(self.start_route)
...
class YazKlinikDesktop(QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        self.setStatusBar(QStatusBar())
        self.statusBar().showMessage("YazKlinik v1000.1.8 Web Kabuk hazır")
    ...
    def _build_native_patient(self) -> QWidget:
        ...
        for text, route, primary in [
            ("Hasta dosyası", "/hasta/{patient}/hasta-dosyasi", True),
            ("Medya arşivi", "/hasta/{patient}/takip-medya", False),
            ("DICOM medya", "/hasta/{patient}/dicom-medya", False),
            ("DICOM / USG Zeka", "/hasta/{patient}/dicom", False),
            ("Resimle Ön Tanı", "/hasta/{patient}/gorsel-tani-destegi", False),
            ("YZ anomali", "/hasta/{patient}/yz-anomali-tarama", False),
            ("PDF editor", "/hasta/{patient}/pdf-editor", False),
            ("Reçete hazırla", "/hasta/{patient}/recete-hazirla", False),
        ]:
...
class YazKlinikWebShell(QMainWindow):
    ...
    def _build_navigation_rail(self) -> QFrame:
        ...
        open_browser = self._rail_button("Tarayıcı", "__browser__", "SP_DialogOpenButton")
    ...
    def _build_native_api_center(self) -> QScrollArea:
        ...
        patient_actions = (
            ...
            ("Reçete", "/hasta/{patient}/recete-hazirla", False),
        ...
        try:
            if patient_key:
                ...
                if hasattr(self, "center_active_title"):
                    ...
                    self.center_active_sub.setText("Hasta dosyası hazır")
        ...
        try:
            ...
            if status:
                self.center_ai_value.setText(
                    "Hazır" if status.get("ollama_online") else "Beklemede")
    ...
    def _build_native_professional_tool_center(self) -> QScrollArea:
        ...
        add_button(3, 0, "Reçete", lambda: f"/hasta/{urllib.parse.quote(self._patient_key_from_route(getattr(self, 'current_professional_tool_route', '')) or '', safe='')}/recete-hazirla")
        add_button(3, 1, "Sesli Reçete", lambda: f"/hasta/{urllib.parse.quote(self._patient_key_from_route(getattr(self, 'current_professional_tool_route', '')) or '', safe='')}/sesli-recete")
        layout.addLayout(grid)
    ...
    def _load_finished(self, ok: bool) -> None:
        ...
        self._set_status("Hazır" if ok else "Sayfa hatası",
                         "ok" if ok else "bad")
        if ok:
            self._apply_web_shell_mask()
        self.statusBar().showMessage(
            "Windows Fluent kabuk hazır" if ok else "Sayfa yüklenemedi",
    ...
    def refresh_server_status(self) -> None:
        if self.connection_mode == "local":
            ...
            try:
                ...
                ollama = "AI hazır" if data.get("ollama_online") else "AI beklemede"
                self._set_status(f"Yerel API | {ollama}", "busy")
                try:
                    self.shell_server_value.setText("Yerel")
                    self.shell_ai_value.setText(
                        "Hazır" if data.get("ollama_online") else "Beklemede")
                    if hasattr(self, "win_api_badge"):
        ...
        try:
            ...
            try:
                ...
                self.shell_ai_value.setText("Hazır" if data.get("ollama_online") else "Beklemede")
                if hasattr(self, "win_api_badge"):
