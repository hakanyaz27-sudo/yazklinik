...
& $Python (Join-Path $Root "D250_TERMINAL_SYNC.py") --server-url $ServerUrl | Out-Null
...
if (-not (Test-YazKlinikReady -Url $ServerUrl)) {
    ...
    Start-Process -FilePath $Python `
        ...
        -WindowStyle Hidden `
        -RedirectStandardOutput (Join-Path $Root "D250_server.log") `
        -RedirectStandardError (Join-Path $Root "D250_server_HATA.log")
    ...
    if (-not $ready) {
        Write-Host "D200 server acilmadi. D250_server_HATA.log dosyasina bakin."
